...
public class CustomerMenu extends Menu {
    ...
    public CustomerMenu(Customer customer, Scanner read) {
        ...
        System.out.printf(ColourManager.ouColour() + "Enter Delivery Address: " + ColourManager.reColour()); // Ask Input
        System.out.print(ColourManager.inColour());
        String address = read.nextLine().trim(); // Input
    ...
    public void makePayment(Scanner read) {
        ...
        if (!promoCodes.isEmpty()) {
            System.out.println(ColourManager.ouColour() + "\n===================================== Promo Code =====================================\n" + ColourManager.reColour()); // Output
            System.out.printf(ColourManager.ouColour() + "Do you want to apply a promo code? (y/n): " + ColourManager.reColour()); // Ask Input
            System.out.print(ColourManager.inColour());
            String applyPromo = read.nextLine().trim().toLowerCase(); // Input
            System.out.print(ColourManager.reColour());
            
            if (applyPromo.equals("y")) {
                while (true) {
                    System.out.printf(ColourManager.ouColour() + "Enter Promo Code (OR 'cancel' to Skip): " + ColourManager.reColour()); // Ask Input
                    System.out.print(ColourManager.inColour());
                    String promoCode = read.nextLine().trim().toUpperCase(); // Input
                    System.out.print(ColourManager.reColour());
                    
                    if (promoCode.equalsIgnoreCase("cancel")) {
                        break;
                    }
                    
                    for (PromoCode promo : promoCodes) {
                        if (promo.getCode().equals(promoCode)) {
                            double discountAmount = order.getTotal() * (promo.getDiscountPercentage() / 100);
                            order.setDiscount(discountAmount);
                            order.setPromoCode(promo.getCode());
                            System.out.println(ColourManager.suColour() + "Promo code applied! Discount: " + String.format("%.2f", discountAmount) + " (" + promo.getDiscountPercentage() + "%)" + ColourManager.reColour()); // Success
                            // Remove one-time use promo code.
                            if (!promo.isPermanent()) {
                                promoCodes.remove(promo);
                                FileManager.savePromoCodes(promoCodes, "promocodes.csv");
                            }
                            break;
                        }
                    }
                    ...
                    if (order.getPromoCode() != null) {
                        ...
                        System.out.println(ColourManager.erColour() + "Invalid promo code. Please try again." + ColourManager.reColour()); // Error
                    }
                }
            }
        }
    ...
    private void printReceipt() {
        ...
        if (order.getPromoCode() != null) {
            receiptLines.add(String.format("Promo Code      : %s", order.getPromoCode()));
        }
        ...
        if (order.getPromoCode() != null) {
            receiptLines.add(String.format("%-66s %19.2f", "DISCOUNT", -order.getDiscount()));
            receiptLines.add(String.format("%-66s %19.2f", "TOTAL AFTER DISCOUNT", order.getDiscountedTotal()));
        }
