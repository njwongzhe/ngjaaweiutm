...
public abstract class MenuItem {
    ...
    protected static int readValidNumber(Scanner read, int min, int max) {
        while (true) {
            try {
                ...
                int value = Integer.parseInt(read.nextLine()); // Input
                System.out.print(ColourManager.reColour());
                if (value >= min && value <= max) {
                    return value;
                }
                System.out.printf(ColourManager.erColour() + "Please Enter a Value Between %d and %d: ", min, max); // Error // Ask Input
                System.out.print(ColourManager.reColour()); 
            } catch (NumberFormatException e) {
                System.out.printf(ColourManager.erColour() + "Invalid Input. Please Enter a Number: " + ColourManager.reColour()); // Error // Ask Input
            }
        }
    }
    ...
    protected static double readValidDouble(Scanner read, double min, double max) {
        while (true) {
            try {
                System.out.print(ColourManager.inColour());
                double value = Double.parseDouble(read.nextLine()); // Input
                System.out.print(ColourManager.reColour());
                if (value >= min && value <= max) {
                    return value;
                }
                System.out.printf(ColourManager.erColour() + "Please Enter a Value Between %.2f and %.2f: ", min, max); // Error // Ask Input
                System.out.print(ColourManager.reColour()); 
            } catch (NumberFormatException e) {
                System.out.printf(ColourManager.erColour() + "Invalid Input. Please Enter a Number: " + ColourManager.reColour()); // Error // Ask Input
            }
    ...
    protected static boolean isNameDuplicate(String name) {
        return allMenuItems.stream().anyMatch(item -> item.getName().toLowerCase().equals(name.toLowerCase()));
    }
    ...
    protected static boolean isValidItemName(String name) {
        ...
        if (name.matches("^[FDCfdc]\\d+$")) {
            System.out.println(ColourManager.erColour() + "Error: Item name cannot be in the format of a menu item ID (F/D/C followed by numbers).\n" + ColourManager.reColour()); // Error
            return false;
        }
        return true;
    }
    ...
    protected static void readCommonDetails(MenuItem item, Scanner read) {
        ...
        while (true) {
            System.out.printf(ColourManager.ouColour() + "Enter Item Name: " + ColourManager.reColour()); // Ask Input
            System.out.print(ColourManager.inColour()); 
            String name = read.nextLine().trim(); // Input
            System.out.print(ColourManager.reColour()); 
            ...
            if (isNameDuplicate(name)) {
                System.out.println(ColourManager.erColour() + "An item with this name already exists!" + ColourManager.reColour()); // Error
                System.out.printf(ColourManager.ouColour() + "\nDo You Want to Try a Different Name? (y/n): " + ColourManager.reColour()); // Ask Input
                System.out.print(ColourManager.inColour());
        ...
        // Read price
        System.out.printf(ColourManager.ouColour() + "Enter Item Price: " + ColourManager.reColour()); // Ask Input
        item.setPrice(readValidDouble(read, 0, Double.MAX_VALUE));
    }

    public void printAvailable(boolean isAdmin) {
        if (isAdmin) {
    ...
    public abstract void printDescription(boolean isAdmin);
