...
public class AdminMenu extends Menu{
    ...
    private ArrayList<PromoCode> promoCodeList = new ArrayList<PromoCode>();

    public AdminMenu() {
        super();
        // Load admins and customers.
        this.admins = FileManager.loadAdmins("admins.csv");
        this.promoCodeList = FileManager.loadPromoCodes("promocodes.csv");
    }
    ...
    public void generateDailySalesReport(Scanner read) {
        ...
        for (Order order : allOrders) {
            ...
            if (orderDate.equals(dateInput) && !order.getPaymentMethod().toLowerCase().equals("unpaid")) {
                ...
                for (OrderItem orderItem : order.getItems()) {
                    ...
                    for (MenuItem item : items) {
                        if (item.getMenuItemID().equals(menuItem.getMenuItemID())) {
                            itemSubtotal = item.getPrice() * orderItem.getQuantity();
                        }
                    }

                    if(orderItem.getMenuItem() instanceof Food) itemType = "FOOD";
                    else if(orderItem.getMenuItem() instanceof Drink) itemType = "DRINK";
                    else if(orderItem.getMenuItem() instanceof Combo) {
                        itemType = "COMBO";
                        Combo combo = (Combo)menuItem;
                        
                        // Parse exchange fees from description lines
                        for (String descLine : combo.getDescriptionLines()) {
                            if (descLine.contains("[extra fee: RM")) {
                                try {
                                    // Extract the fee value from the description
                                    String feePart = descLine.split("\\[extra fee: RM")[1];
                                    String feeStr = feePart.split("\\]")[0];
                                    double fee = Double.parseDouble(feeStr);
                                    totalExchangeFees += fee * orderItem.getQuantity();
                                } catch (Exception e) {
                                    System.out.println("Warning: Error parsing exchange fee from: " + descLine);
                                }
                            }
                        }
                    }
                    ...
                    salesItems.add(new SalesItem(menuItem.getMenuItemID(), menuItem.getName(), orderItem.getQuantity(), itemSubtotal, itemType));
                }
            }
        }
        ...
        if (totalExchangeFees > 0) {
            System.out.printf("%-54s %14.2f\n", "TOTAL EXCHANGED ITEMS EXTRA FEES", totalExchangeFees);
        }
        if (totalDiscount > 0) {
        ...
        System.out.println("----------------------------------------------------------------------");
        System.out.printf("%-54s %14.2f\n", "NET REVENUE", grandTotal + totalExchangeFees - totalDiscount);
        System.out.println("----------------------------------------------------------------------");
    ...
    private void managePromoCodes(Scanner read) {
        while (true) {
            System.out.println(ColourManager.ouColour() + "\n============= Manage Promo Codes =============\n" + ColourManager.reColour()); // Output
            System.out.println(ColourManager.ouColour() + "<Promo Code Management>" + ColourManager.reColour());
            System.out.print(ColourManager.opColour());
            System.out.println("0. Back to Main Menu"); // Option
            System.out.println("1. Generate Permanent Promo Code");
            System.out.println("2. Generate One-Time Use Promo Codes (Batch)");
            System.out.println("3. View Existing Promo Codes");
            System.out.println("4. Delete Promo Code");
            System.out.print(ColourManager.reColour());
            
            System.out.printf(ColourManager.ouColour() + "Select an Option (0 - 4): " + ColourManager.reColour()); // Ask Input
            System.out.print(ColourManager.inColour());
            String choice = read.nextLine(); // Input
            System.out.print(ColourManager.reColour());
            ...
            try {
                switch (choice) {
                    case "0":
                        ...
                        return;
                    case "1":
                        System.out.println(ColourManager.ouColour() + "\n======= Generate Permanent Promo Code ========\n" + ColourManager.reColour()); // Output
                        generateSinglePromoCode(read, true);
                        break;
                    case "2":
                        System.out.println(ColourManager.ouColour() + "\n======== Generate One-Time Promo Code ========\n" + ColourManager.reColour()); // Output
                        generateBatchPromoCodes(read);
                        break;
                    case "3":
                    ...
                    case "4":
                        System.out.println(ColourManager.ouColour() + "\n============= Delete Promo Code ==============\n" + ColourManager.reColour()); // Output
                        deletePromoCode(read);
                        break;
                    default:
                        System.out.println(ColourManager.erColour() + "Invalid option. Please try again." + ColourManager.reColour());  // Error
                }
            } catch (Exception e) {
                System.out.println(ColourManager.erColour() + "Error: " + e.getMessage() + ColourManager.reColour());  // Error
            }
        }
    }

    private void generateSinglePromoCode(Scanner read, boolean isPermanent) {
        try {
            ArrayList<PromoCode> existingPromoCodes = FileManager.loadPromoCodes("promocodes.csv");
            System.out.printf(ColourManager.ouColour() + "Enter Discount Percentage (1 - 100): " + ColourManager.reColour());  // Ask Input
            System.out.print(ColourManager.inColour());
            double discount = Double.parseDouble(read.nextLine()); // Input
            System.out.print(ColourManager.reColour());
            
            if (discount < 1 || discount > 100) {
                throw new IllegalArgumentException("\nDiscount must be between 1% and 100%."); // Error
            }

            String code = PromoCode.generatePromoCode(existingPromoCodes);
            PromoCode promo = new PromoCode(code, discount, isPermanent);
            promoCodeList.add(promo);
            FileManager.savePromoCodes(promoCodeList, "promocodes.csv");
            
            System.out.println(ColourManager.suColour() + "Promo Code Generated: " + code + " (" + (isPermanent ? "Permanent" : "One-Time Use") + ", " + discount + "% Discount)" + ColourManager.reColour()); // Success
        } catch (NumberFormatException e) {
            System.out.println(ColourManager.erColour() + "\nInvalid number format. Please enter a valid percentage." + ColourManager.reColour());  // Error
        } catch (IllegalArgumentException e) {
            System.out.println(ColourManager.erColour() + e.getMessage() + ColourManager.reColour());  // Error
        }
    }

    private void generateBatchPromoCodes(Scanner read) {
        try {
            ArrayList<PromoCode> existingPromoCodes = FileManager.loadPromoCodes("promocodes.csv");
            System.out.printf(ColourManager.ouColour() + "Enter Number of Promo Codes to Generate: " + ColourManager.reColour());  // Ask Input
            System.out.print(ColourManager.inColour());
            int count = Integer.parseInt(read.nextLine()); // Input
            System.out.print(ColourManager.reColour());
            ...
            if (count <= 0) {
                throw new IllegalArgumentException("\nCount must be positive."); // Error
            }
            if (discount < 1 || discount > 100) {
                throw new IllegalArgumentException("\nDiscount must be between 1% and 100%."); // Error
            }

            System.out.println(ColourManager.suColour() + "Generating " + count + " one-time use promo codes with " + discount + "% discount..." + ColourManager.reColour()); // Success
            
            for (int i = 0; i < count; i++) {
                String code = PromoCode.generatePromoCode(existingPromoCodes);
                PromoCode promo = new PromoCode(code, discount, false);
                promoCodeList.add(promo);
                System.out.println(code);
            }
            
            FileManager.savePromoCodes(promoCodeList, "promocodes.csv");
            System.out.println(ColourManager.suColour() + "Batch generation completed." + ColourManager.reColour()); // Success
        } catch (NumberFormatException e) {
            System.out.println(ColourManager.erColour() + "\nInvalid number format. Please enter valid numbers." + ColourManager.reColour()); // Error
        } catch (IllegalArgumentException e) {
            System.out.println(ColourManager.erColour() + e.getMessage() + ColourManager.reColour()); // Error
        }
    }

    private void viewPromoCodes() {
        if (promoCodeList.isEmpty()) {
            System.out.println(ColourManager.erColour() + "No promo codes available." + ColourManager.reColour()); // Error
            return;
        }
    ...
    private void deletePromoCode(Scanner read) {
        System.out.printf(ColourManager.ouColour() + "Enter Promo Code to Delete: " + ColourManager.reColour()); //  Ask Input
        System.out.print(ColourManager.inColour());
        String code = read.nextLine().toUpperCase(); // Input
        System.out.print(ColourManager.reColour());

        boolean found = promoCodeList.removeIf(p -> p.getCode().equals(code));
        
        if (found) {
            FileManager.savePromoCodes(promoCodeList, "promocodes.csv");
            System.out.println(ColourManager.suColour() + "Promo code deleted successfully." + ColourManager.reColour()); // Success
        } else {
            System.out.println(ColourManager.erColour() + "\nPromo code not found." + ColourManager.reColour()); // Error
        }
    }
    ...
    public void printMenuOption() {
        ...
        System.out.println("6. Manage Promo Codes");
        System.out.println("7. Filter Orders by Date");
        System.out.println("8. View Most Popular Items");
        System.out.println("9. Generate Daily Sales Report");
        System.out.print(ColourManager.reColour());
    }
        ...
        do {
            ...
            try {
                ...
                switch (choice) {
                    ...
                    case 2:
                        System.out.println(ColourManager.ouColour() + "\n================ Search Item =================\n" + ColourManager.reColour()); // Output
                        System.out.printf(ColourManager.ouColour() + "Enter the Item ID or Name You Want to Find: " + ColourManager.reColour()); // Ask Input
                        System.out.print(ColourManager.inColour());
                    ...
                    case 5:
                        ...
                        System.out.println(ColourManager.ouColour() + "==============================================" + ColourManager.reColour()); // Output
                        break;
                    case 6:  // New case for promo code management.
                        managePromoCodes(read);
                        System.out.println(ColourManager.ouColour() + "==============================================" + ColourManager.reColour()); // Output
                        break;
                    case 7:
                        System.out.println(ColourManager.ouColour() + "\n================ Order Filter ================\n" + ColourManager.reColour()); // Output
